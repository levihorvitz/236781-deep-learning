...
import copy
from tranformer_helpers import *
...
def sliding_window_attention(q, k, v, window_size, padding_mask=None):
    ...
    num_heads = q.shape[1]
    q, k, v, padding_mask = qkv_window_size_padding(q, k, v, window_size, padding_mask)
    v = v.transpose(1, 2)
    new_sequence_length = q.shape[-2]
    scores = qk_sliding_blocks(q, k, window_size, padding_value=-9e15).view(
        batch_size, num_heads, new_sequence_length, 2 * window_size + 1
    )

    if padding_mask is not None:
        padding_mask = torch.logical_not(
            padding_mask.unsqueeze(dim=1).unsqueeze(dim=-1)
        )
        padding_mask = padding_mask.type_as(q).masked_fill(padding_mask, -9e15)
        ones = padding_mask.new_ones(size=padding_mask.size())
        d_mask = qk_sliding_blocks(
            ones, padding_mask, window_size, padding_value=-9e15
        ).view(batch_size, 1, new_sequence_length, 2 * window_size + 1)
        scores += d_mask
    attention = torch.nn.functional.softmax(
        scores / math.sqrt(embed_dim), dim=-1
    ).transpose(1, 2)
    values = pv_sliding_blocks(
        attention, v, window_size
    )
    attention = attention.transpose(2, 1).view(
        batch_size * num_heads, new_sequence_length, 2 * window_size + 1
    )
    attention = populate_diags(attention).view(
        batch_size, num_heads, new_sequence_length, new_sequence_length
    )
    if new_sequence_length != sequence_length:
        padding = new_sequence_length - sequence_length
        padding_left, padding_right = (padding // 2, padding // 2) if padding > 1 else (0, 1)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, padding_mask):
        ...
        # ====== YOUR CODE: ======
        attn_out = self.dropout(self.self_attn(x, padding_mask))
...
class Encoder(nn.Module):
    ...
    def forward(self, sentence, padding_mask):
        ...
        output = self.classification_mlp(encoded).squeeze(-1)
        # ========================
