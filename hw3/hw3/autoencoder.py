...
class EncoderCNN(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        for i in range(len(channels) - 1):
            modules.append(
                nn.Conv2d(
                    in_channels=channels[i], out_channels=channels[i + 1], kernel_size=5
                )
            )
...
class VAE(nn.Module):
    ...
    def encode(self, x):
        ...
        # ====== YOUR CODE: ======
        h = torch.reshape(self.features_encoder(x), (x.shape[0], -1))
        mu = self.calc_mean(h)
        log_sigma2 = self.calc_log_var(h)
        z = mu + torch.randn(mu.size(), device=mu.device) * torch.exp(log_sigma2 * 0.5)
        # ========================
    ...
    def decode(self, z):
        ...
        # ====== YOUR CODE: ======
        reverse_h = self.reverse_transformation(z)
        reverse_h = torch.reshape(reverse_h, (-1, *self.features_shape))
        x_rec = self.features_decoder(reverse_h)
        # ========================
...
def vae_loss(x, xr, z_mu, z_log_sigma2, x_sigma2):
    ...
    # ====== YOUR CODE: ======
    divide_factor = x_sigma2 * torch.numel(torch.FloatTensor(x.shape))
    ...
    kldiv_loss = (trace + norm2_z_mu - det_log) / x.shape[0] - z_mu.shape[1]
