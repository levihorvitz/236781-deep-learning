...
class RNNTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # ====== YOUR CODE: ======
        out, self.state_h = self.model(x, self.state_h)
...
class VAETrainer(Trainer):
    def train_batch(self, batch) -> BatchResult:
        ...
        self.optimizer.zero_grad()
        loss, data_loss, _ = self.loss_fn(x, xr, mu, sigma_2)
        loss.backward()
        self.optimizer.step()
        # ========================
...
class TransformerEncoderTrainer(Trainer):
    def train_batch(self, batch) -> BatchResult:
        ...
        loss.backward()
        self.optimizer.step()
        y_preds = torch.round(torch.sigmoid(out))
        num_correct = (y_preds == label).sum()
        # ========================
...
class FineTuningTrainer(Trainer):
    def train_batch(self, batch) -> BatchResult:
        ...
        self.optimizer.zero_grad()
        loss, out = out[0], out[1]
        loss.backward()
        self.optimizer.step()
        y_preds = torch.argmax(torch.sigmoid(out),dim=1)
        num_correct = (y_preds == labels).sum().detach()
        # ========================
