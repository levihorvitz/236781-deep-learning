...
class CNN(nn.Module):
    ...
    def _make_feature_extractor(self):
        ...
        for p in range(len(self.channels)):
            layers.append(nn.Conv2d(in_channels, self.channels[p], **self.conv_params))
            layers.append(ACTIVATIONS[self.activation_type](**self.activation_params))
            if (p + 1) % self.pool_every == 0:
                layers.append(POOLINGS[self.pooling_type](**self.pooling_params))
            in_channels = self.channels[p]
        # ========================
    ...
    def _make_mlp(self):
        ...
        mlp: MLP = None
        # ====== YOUR CODE: ======
        in_dim = self.hidden_dims + [self.out_classes]
        non_lins = [ACTIVATIONS[self.activation_type](**self.activation_params)] * (
            len(in_dim) - 1
        ) + [nn.Identity()]
        mlp = MLP(self._n_features(), in_dim, non_lins)
        # ========================
        return mlp
    ...
    def forward(self, x: Tensor):
        ...
        out_conv = out_conv.view(out_conv.shape[0], -1)
...
class ResidualBlock(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for out_channels, kernel_size in zip(channels, kernel_sizes):
            ...
            if conv_layer_num < len(channels):
                if dropout > 0:
        ...
        if orig_in_channels != channels[-1]:
            self.shortcut_path.append(
                nn.Conv2d(orig_in_channels, channels[-1], 1, bias=False)
            )
    ...
    def forward(self, x: Tensor):
        ...
        out += self.shortcut_path(residual)
        # ========================
        out = torch.relu(out)
        return out
...
class ResNet(CNN):
    ...
    def _make_feature_extractor(self):
        ...
        # ====== YOUR CODE: ======
        pooling_layers = len(self.channels) // self.pool_every
        extra_layers = len(self.channels) % self.pool_every
        for p in range(pooling_layers):
            res_block_channels = self.channels[
                p * self.pool_every : (p + 1) * self.pool_every
            ]
            if self.bottleneck and in_channels == res_block_channels[-1]:
                layers.append(
        ...
        if extra_layers > 0:
            extra_channels = self.channels[self.pool_every * pooling_layers :]
            if self.bottleneck and in_channels == res_block_channels[-1]:
                layers.append(
                    ResidualBottleneckBlock(
                        in_channels,
                        extra_channels[1:-1],
                        [3] * (len(extra_channels) - 2),
                        batchnorm=self.batchnorm,
                        dropout=self.dropout,
                        activation_type=self.activation_type,
                        activation_params=self.activation_params,
