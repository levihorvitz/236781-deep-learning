...
class Linear(Layer):
    ...
    def __init__(self, in_features, out_features, wstd=0.1):
        ...
        # ====== YOUR CODE: ======
        self.w = torch.normal(
            mean=0, std=wstd, size=(self.out_features, self.in_features)
        )
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        # ====== YOUR CODE: ======
        loss = -x.gather(1, y.view(-1, 1)) + torch.log(torch.exp(x).sum(dim=1))
        # ========================
