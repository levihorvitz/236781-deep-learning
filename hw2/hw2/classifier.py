...
class BinaryClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        ...
        #  Output should be a (N,) integer tensor.
        # ====== YOUR CODE: ======
        return torch.where(y_proba[:, self.positive_class] >= self.threshold, 1, 0).to(
            torch.int32
        )
        # ========================
...
def plot_decision_boundary_2d(
    ...
    x1_grid, x2_grid = torch.meshgrid(
        torch.arange(x[:, 0].min() - 1, x[:, 0].max() + 1, dx),
        torch.arange(x[:, 1].min() - 1, x[:, 1].max() + 1, dx),
    )
    x_grid_stack = torch.stack([x1_grid.flatten(), x2_grid.flatten()], dim=1)
    y_hat = classifier.classify(x_grid_stack).reshape(x1_grid.shape)
    # ========================
...
def select_roc_thresh(
    ...
    fpr, tpr, thresh = roc_curve(y.numpy(), y_pred.detach().numpy())
