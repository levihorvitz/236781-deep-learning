...
def mlp_experiment(
    ...
    model = BinaryClassifier(
        MLP(2, [width] * depth + [2], ["relu"] * depth + ["sigmoid"])
    )
    trainer = ClassifierTrainer(
        model,
        torch.nn.CrossEntropyLoss(),
        torch.optim.Adam(params=model.parameters(), lr=0.01, weight_decay=1e-3),
    )
    ...
    test_epoch_result = trainer.test_epoch(dl_test, verbose=False)
...
def cnn_experiment(
    ...
    **kw,
    ...
    channels = [
        block_filter
        for block_filter in filters_per_layer
        for _ in range(layers_per_block)
    ]
    if model_type == "cnn":
        params = {
            "in_size": next(iter(dl_train))[0][0].shape,
            "out_classes": 10,
            "channels": channels,
    ...
    trainer = ClassifierTrainer(ArgMaxClassifier(model=model), torch.nn.CrossEntropyLoss(), torch.optim.Adam(params=model.parameters(), lr=lr), device)
