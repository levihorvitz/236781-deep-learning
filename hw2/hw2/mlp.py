...
class MLP(nn.Module):
    ...
    def __init__(
        ...
        for out_dim, act_idx in zip(dims, nonlins):
            self.model.append(nn.Linear(in_dim, out_dim))
            if act_idx in ACTIVATION_DEFAULT_KWARGS:
                activation = ACTIVATIONS[act_idx](**ACTIVATION_DEFAULT_KWARGS[act_idx])
            else:
                activation = ACTIVATIONS[act_idx]() if type(act_idx) == str else act_idx
