...
class LinearClassifier(object):
    def __init__(self, n_features, n_classes, weight_std=0.001):
        ...
        self.weights = None
        # ====== YOUR CODE: ======
        self.weights = torch.empty(n_features, n_classes).normal_(std=weight_std)
        # ========================
    ...
    def train(
        ...
        for epoch_idx in range(max_epochs):
            ...
            for batch_x, batch_y in dl_train:
                y_predicted, x_scores = self.predict(batch_x)
                total_correct += self.evaluate_accuracy(batch_y, y_predicted)
                average_loss += loss_fn.loss(batch_x, batch_y, x_scores, y_predicted)
                self.weights -= (
                    learn_rate * loss_fn.grad() + weight_decay * self.weights
                )
