...
class LinearRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        # ====== YOUR CODE: ======
        N, C = X.shape
        regularization_term = N * self.reg_lambda * np.eye(C)
        regularization_term[0][0] = 0
        w_opt = np.linalg.inv(X.T @ X + regularization_term) @ X.T @ y
        # ========================
...
def top_correlated_features(df: DataFrame, target_feature, n=5):
    ...
    # ====== YOUR CODE: ======
    df_corr = df.corr(method="pearson")[target_feature]
    # remove self corr from the list
    df_corr = df_corr.drop(target_feature, axis=0)
    abs_sorted_df_corr = df_corr.abs().sort_values(ascending=False)[:n]
    top_n_features = list(abs_sorted_df_corr.index)
    top_n_corr = list(df_corr[top_n_features])
    # ========================
